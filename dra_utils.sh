...
add_criterial_rule_to_dra() {
    ...
    debugme echo -e "curl -k -H Content-Type:application/json -H projectKey:$DRA_PROJECT_KEY -X POST -d @$CRITERIAL_FILE $DRA_ADD_CRITERIAL_URL"
    curl -k -H Content-Type:application/json -H projectKey:$DRA_PROJECT_KEY -X POST -d @$CRITERIAL_FILE $DRA_ADD_CRITERIAL_URL > "$RESPONSE_FILE"
    local RC=$?
    debugme echo -e $(cat "$RESPONSE_FILE")
    echo ""
    if [ $RC == 0 ]; then
